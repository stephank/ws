...
function completeUpgrade(req, res, version, cb) {
  ...
  res.switchProtocols(function(socket, upgradeHead) {
    socket.setTimeout(0);
...
function handleHixieUpgrade(req, res, cb) {
  ...
  function switchCallback(socket, upgradeHead) {
    socket.setTimeout(0);
    socket.setNoDelay(true);

    // retrieve nonce
    var remaining = 8;
    function nonceHandler(data) {
    ...
    socket.addListener('data', nonceHandler);
    nonceHandler(upgradeHead);
  }
